...
import company from '../../../../../../assets/avatar-company.png';
class OrgVersionCfgDetailSider extends React.PureComponent {
  constructor(props) {
    super(props);
 ...
 */
  ...
  */
  ...
  render() {
    const { menuData, toggleCollapsed, location: { pathname }, orgVersionCfgDetail: { versionData= '' } } = this.props;
    // 处理菜单路径的后缀
    const finalMenuData = [];
    if (suffix) {
    ...
    const vna = versionDataJson.versionName || '--';
    const vnb= versionDataJson.orgName || '--';
    const vnc= versionDataJson.examClassName || '--';
    const all = vna+vnb+vnc;    
     return (
        ...
        >
          ...
          <div className="scroll-wrap" style={{ height: '100%' }}>
            <div style={this.state.collapsed ? { margin: '20px 0', textAlign: 'center', display: 'none' } : { margin: '20px 0', textAlign: 'center' }}>
              ...
              {versionDataJson.versionName || '--'}版本<br/><img src={company} alt="" style={{ height: '360',borderRadius:'50%',overflow:"hidden",width:"80px",height:"80px"}} /><br/>{versionDataJson.orgName || '--'}<br/>
           <Tooltip placement='bottom' title={versionDataJson.examClassName|| '--'} overlayClassName="tooltipColor">
              <span style={{
                maxWidth: '150px',
                maxHeight: '150px', whiteSpace: 'nowrap',  textOverflow: 'ellipsis', overflow: 'hidden',
                display: 'inline-block',
              }}>
                    {versionDataJson.examClassName || '--'}
...
export default connect(({ orgVersionCfgDetail }) => ({
  orgVersionCfgDetail,
}))(OrgVersionCfgDetailSider);
