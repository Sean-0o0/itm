...
const prefix = '';
class OrgVersionCfgDetailLayout extends React.PureComponent {
  constructor(props) {
    super(props);
  ...
  render() {
    const { theme, route = {}, userBusinessRole, location, hasAuthed, orgVersionCfgDetail } = this.props;
    const routes = lodash.get(route, 'routes', []);
    const { versionData, menuData } = orgVersionCfgDetail;
...
export default connect(({ global, orgVersionCfgDetail }) => ({
