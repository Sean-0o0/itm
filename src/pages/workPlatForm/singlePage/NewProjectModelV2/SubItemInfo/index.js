...
class SubItemInfo extends Component {
  ...
  callbackData = () => {
    ...
    tableData.map(item => {
      ...
      let obj = {
        ...
        XMJL: ['', undefined, null, ' '].includes(item['SUBXMJL' + item.ID])
          ? '-1'
          : item['SUBXMJL' + item.ID],
        XMLX: ['', undefined, null, ' '].includes(item['SUBXMLX' + item.ID])
          ? '-1'
          : String(item['SUBXMLX' + item.ID]),
        // SUBGLRJ: item['SUBGLRJ' + item.ID],
        YYBM: String(
          ['', undefined, null, ' ', 'null'].includes(item['SUBYYBM' + item.ID])
            ? []
            : item['SUBYYBM' + item.ID],
        ),
        CGFS: String(
          ['', undefined, null, ' '].includes(item['SUBCGFS' + item.ID])
            ? '-1'
        ...
        //项目预算金额/三个金额之和
        XMYS:
          String(item['SUBSFBHYJ' + item.ID]) === '2'
            ? String(item['SUBYSJE' + item.ID] === '' ? '0' : item['SUBYSJE' + item.ID])
            : String(item['SUBYSJE-TOTAL' + item.ID]),
  ...
  fetchQuerySubProjectsInfo = () => {
    ...
    FetchQuerySubProjectsInfo({ parentId: xmid })
      .then(result => {
          ...
          if (data.length > 0) {
            for (let i = 0; i < data.length; i++) {
              ...
              arr.push({
                ...
                ['SUBXMJL' + data[i]?.ID]:
                  String(data[i]?.XMJL) === '-1' ? null : String(data[i]?.XMJL),
                ...
                ['SUBYSJE-TOTAL' + data[i]?.ID]:
                  Number(data[i]?.RJYSJE) + Number(data[i]?.KJCGJE) + Number(data[i]?.DDCGJE),
              });
            }
          } else {
            arr.push({
  ...
  itemChange = (e, record, index, key) => {
    ...
    if (key === 'SUBRJYSJE' || key === 'SUBKJCGJE' || key === 'SUBDDCGJE') {
      tableData.map(item => {
        if (item.ID === record.ID) {
          item['SUBYSJE-TOTAL' + item.ID] =
            Number(item['SUBRJYSJE' + item.ID]) +
            Number(item['SUBKJCGJE' + item.ID]) +
            Number(item['SUBDDCGJE' + item.ID]);
        }
      });
      ...
      if (haveHard == '1') {
        ...
        if (
          ...
          Number(softBudget) + Number(frameBudget) + Number(singleBudget)
        ) {
    ...
    const tableColumns = [
                  ...
                  staffList.map((item, index) => {
                    ...
                    return (
                      <Select.Option key={index} value={item.id}>
                        {item.name}({item.orgName ? item.orgName : loginUser.orgName})
                      </Select.Option>
                    );
                  })}
              </Select>
        ...
        render(text, record, index) {
          return (
            <TreeSelect
              ...
              showSearch
              disabled={
                projectTypeZY.filter(item => item.ID == record['SUBXMLX' + record.ID]).length > 0
              }
      ...
      },
      {
        ...
        render(text, record, index) {
          return (
            <InputNumber
              disabled={
                record['SUBSFBHYJ' + record.ID] === '' || record['SUBSFBHYJ' + record.ID] === '1'
              }
              style={{ width: '100%' }}
      ...
      },
      {
        ...
        render(text, record, index) {
          return (
            <Select
              disabled={record['SUBXMLX' + record.ID] != '1'}
              value={record['SUBSFBHYJ' + record.ID]}
              style={{ width: 120 }}
      ...
      // },
      {
        ...
        render(text, record, index) {
          return (
            <InputNumber
              disabled={
                record['SUBSFBHYJ' + record.ID] === '' || record['SUBSFBHYJ' + record.ID] === '2'
              }
              style={{ width: '100%' }}
        ...
        render(text, record, index) {
          return (
            <InputNumber
              ...
              value={record['SUBKJCGJE' + record.ID]}
              style={{ width: '100%' }}
        ...
        render(text, record, index) {
          return (
            <InputNumber
              ...
              value={record['SUBDDCGJE' + record.ID]}
              style={{ width: '100%' }}
