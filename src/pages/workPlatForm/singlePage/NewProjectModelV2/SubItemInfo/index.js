  ...
  callbackData = () => {
    ...
    tableData.map((item) => {
      ...
      let obj = {
        ...
        XMLX: ['', undefined, null,' '].includes(item['SUBXMLX' + item.ID]) ? '-1' : String(item['SUBXMLX' + item.ID]),
        // SUBGLRJ: item['SUBGLRJ' + item.ID],
        YYBM: String(['', undefined, null,' ', 'null'].includes(item['SUBYYBM' + item.ID]) ? [] : item['SUBYYBM' + item.ID]),
