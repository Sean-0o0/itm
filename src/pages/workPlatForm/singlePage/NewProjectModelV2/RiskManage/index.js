...
class RiskManage extends Component {
    ...
    render() {
        ...
        return (<div className='risk-body'>
            ...
            <div className='risk-cont' ref={(el) => { this.messagesEnd = el; }}>
                {data.map((item, index) => {
                    return (
                        <DataItem onRef={(ref)=>this.onRef(ref,index)} removeRisk={this.removeRisk} data={item} key={index} order={index} total={data.length} />
                    )
                })
                }
            </div>
