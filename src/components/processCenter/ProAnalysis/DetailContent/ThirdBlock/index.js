...
class ThirdBlock extends React.Component {
  ...
  render() {
    ...
    const {lastUserAnalysis= [], showProcess=false} = this.props;

    const colums = showProcess?this.fetchProColums():this.fetchColums();
    let xAxisName = lastUserAnalysis.map(m => `${m.stepName}`);
    let xAxisData = lastUserAnalysis.map(m => `${m.executes}`);
    const option = {
      yAxis: {
        type: 'category',
        inverse: true,//倒叙
    ...
    return (
      <Fragment>
        <Row style={{ paddingBottom: 20 }}>
          <div className="factor-content-title"><div className="tip"></div>{showProcess?"频度分析":"最近执行对象分析"}</div>
...
export default Form.create()(ThirdBlock);
