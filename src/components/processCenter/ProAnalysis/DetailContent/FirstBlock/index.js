...
class FirstBlock extends React.Component {
  ...
  render() {
    const { elaAnalysis= [], showProcess = false} = this.props;
    let xAxisName = showProcess?elaAnalysis.map(m => `${m.stepName}`):elaAnalysis.map(m => `${m.wfName}`);
    let xAxisData = elaAnalysis.map(m => `${m.elapsed}`);
    ...
    return (
      <Fragment>
        <Row>
          <div className="factor-content-title"><div className="tip"></div>总耗时分析</div>
...
export default Form.create()(FirstBlock);
