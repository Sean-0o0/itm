...
class BasicIndexTable extends React.Component {
  ...
  cmptChange = (column, pos, value, type) => {  //colum: 列字段名 pos :行位置 value:单元格显示数据 type:组件类型
    ...
    if (type) {
      handleChangeData && handleChangeData(data, pos, type)
    } else {
      handleChangeData && handleChangeData(data, pos, "")
    }
  }
  ...
  getLabel = (str, label) => {
    ...
    if (label === '1') {
      ...
      if (str) {
        html =
          <div style={{textAlign: 'left'}} dangerouslySetInnerHTML={{__html: str.replace(/\n/g, '<br/>') || ''}}></div>;
      }
    } else {
      ...
      return {
        ...
        render: (value, row, index) => {
          const obj = {
            ...
            children:
              <div>{this.chooseComponent(option, dataIndex, value, type, index, data, label, edit, selectedWidth)}</div>,
            props: {
              rowSpan: 0,
              colSpan: 1,
              align: columnAlign
      ...
      columnResult.push({
        ...
        render: (value, row, index) => {
          return this.state.data.length >= 1 ?
            <React.Fragment>
              {value.includes(1) &&
              <Popconfirm title={`确定退回吗?`} onConfirm={() => this.handleDelete(row, index)}>
                <Tooltip title='退回'><a><i className='iconfont icon-back-arrow'
                                          style={{fontSize: '18px', margin: '0.2rem'}}/></a></Tooltip>
              </Popconfirm>
              }
              {value.includes(2) &&
              <Tooltip title='修改'><a><i className='iconfont icon-edit' style={{fontSize: '18px', margin: '0.2rem'}}
                                        onClick={() => this.update(row)}/></a></Tooltip>}
              {/*{value.length === 0 && <span style={{textAlign: 'center'}}>--</span>}*/}
            </React.Fragment>
            : null
        }
      })
