  ...
  render() {
    ...
    return (
      <div className='pos-r flex-r' style={{ alignItems: 'center' }}>
        ...
        <Progress type='dashboard' percent={this.getPercent(TOTALTASKS)}
          format={() =>
            <div style={{ color: this.handleComplete(GROUPSTATE).color, fontWeight: 'bold' }}><CountUp start={0} end={this.getCOMPLTASKS()} duration="3" />/{TOTALTASKS}
              <br />
              <span className='fs16' style={{ fontWeight: 'normal',verticalAlign: '30%' }}>{GROUPSTATENM}</span>
            </div>}
