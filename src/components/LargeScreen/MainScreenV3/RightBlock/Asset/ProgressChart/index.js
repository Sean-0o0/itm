...
export class ProgressChart extends Component {
  ...
  getPercent = (data = {}) => {
    const { COMPLTSTEPNUM } = this.state;
    ...
    return (
      <div className='pos-r flex-r' style={{ alignItems: 'center' }}>
        ...
        <Progress type='dashboard' percent={this.getPercent(item)}
                  format={() => <div
                    style={{
                      color: `${this.handleComplete(item).color}`,
                      fontWeight: 'bold',
                    }}><CountUp start={0} end={this.getCOMPLTSTEPNUM()} duration="3" />/{item.GROUPSTEPNUM ? item.GROUPSTEPNUM : '-'}<br /><span
                    className='fs16'
                    style={{ fontWeight: 'normal', verticalAlign: '40%' }}>{this.handleComplete(item).status}</span>
                  </div>}
