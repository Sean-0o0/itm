...
class NoTitleStyleModuleChart extends React.Component {
    state = {
    ...
    getBoxContent = () => {
        ...
        const { records = [], tClass = '', businessUnit = false } = this.props;
                ...
                {records.map((data, index) => (
                    <React.Fragment key={index}>
                        <div className="box-title" style={{ display: index !== currentIndex && 'none' }}>
                          {
                            businessUnit ? ( <div className={titleClass} style={{fontWeight: '800', color: '#fff'}}></div>)
                              :
                              (<div className={titleClass} style={{fontWeight: '800', color: '#fff'}}>{data.chartTitle || ''}</div>)
                          }
                        </div>
                        <div className="chart-box" style={{ display: index !== currentIndex && 'none', padding: records.length === 1 ? '0' : '0 2.333rem' }}>
    ...
    getChartComponent = data => {
        ...
        const { indexConfig = {}, dispatch, businessUnit = false } = this.props;
        let com = null;
        switch (chartType) {
            case '1':
                com = '定制类（不支持个性化配置）';
    ...
    render() {
        ...
        const { records = [] } = this.props;
        return (
            <div className="h100 flex-c" onMouseEnter={this.stopAutoplay} onMouseLeave={this.startAutoplay}>
                {records.length > 1 ? (
                    <React.Fragment>
                        <a
                            className="btn-left"
...
export default NoTitleStyleModuleChart;
