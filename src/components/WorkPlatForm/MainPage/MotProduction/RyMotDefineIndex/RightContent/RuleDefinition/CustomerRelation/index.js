...
import { Row, message, Table, Input, Form, Select, Button } from 'antd';
  ...
  componentWillReceiveProps(nextProps) {
    ...
    onOperate = (text, record) => {
      ...
      const payload = {
        ...
        objId: `${staffId}|${record.GXLXID}`, // 查询值  员工ID|关系类型ID
        objTp: 3, // 查询类型 查询类型：0|生效的规则;1|默认;2|营业部;3|关系类型;4|客户
      };
      ...
      FetchqueryStaffMotEventInfo(payload).then((res) => {
        ...
        if (code > 0) {
          ...
          if (jsonData.length > 0) {
            jsonData.map((item) => {
              const obj = {
                gxlx: item.GXLXMC ? item.GXLXMC : '--',
                GXLXID: item.GXLXID ? item.GXLXID : '--',
              };
              data.push(obj);
            });
            this.setState({
        ...
        if (selectAll) {
          dataSource.map((dataItem) => {
            ...
            selectedRows.map((selectItem) => {
              if (dataItem.GXLXID !== selectItem.GXLXID) {
                total += 1;
              }
              if (total === selectedRows.length) {
                chcidArr.push(dataItem.GXLXID);
              }
            });
          });
        } else {
    ...
    render() {
      ...
      const { dataSource = [], selectedRowKeys = [], visible, staffData = [], addMode, gxlx, czType } = this.state;
      const rowSelection = {
        selectedRowKeys,
      ...
      return (
        <Fragment>
          <Row>
            <Row>
              <div className="mot-fbgz-color" style={{  borderTopLeftRadius: '8px', borderTopRightRadius: '8px', lineHeight: '40px' }}>
                <span style={{ marginLeft: '40px', color: '#FFF' }}>客户关系定义</span>
              </div>
            </Row>
          ...
          <BasicModal {...modalProps}>
            <CommonModalContent addMode={addMode} czType={czType} gxlx={gxlx} staffData={staffData} motDetail={motDetail} dictionary={dictionary} wrappedComponentRef={form => this.formRef = form} />
          </BasicModal>
        </Fragment>
      );
    }
...
export default Form.create()(CustomerRelation);
