...
class EditableCell extends React.Component {
  ...
  renderCell = () => {
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      if(record.isRvs === '1' && record.type === 2){
        Reflect.set(record, `std1Score`, '');
      }
      if (element !== undefined && record.isRvs === '1' && record.type === 1) {
        const node = (
      ...
      childNode = dataIndex === 'examWeight' || (editing && !typeArr[record.type - 1].includes(dataIndex)) ? (
        <Form.Item style={{ margin: 0 }}>
          <div className="df">
            ...
            }{dataIndex === 'zeroThld' || dataIndex === 'pctThld' || dataIndex === 'elgThld' ? <span style={{ lineHeight: '32px' }}> {record.pctThldUnit || ''}</span> : ''}
          </div>
        </Form.Item>
      ) : (
          <div
          ...
          // onClick={record.isRvs === '1' ? this.toggleEdit : ()=>{}}
          >
            {children[2]}
          </div>
        );
