...
class LeftTable extends Component {
  ...
  componentWillReceiveProps(nextProps, nextContext) {
    // console.log("columnsMinStatus",nextProps.columnsMinStatus)
  ...
  componentWillMount() {
    ...
    // })
    // let leftColumns;
    // if (this.props.columnsMinStatus) {
    //   leftColumns = this.structLeftColumns();
    // } else {
    //   leftColumns = this.structMinleftColumns();
    // }
    ...
    const columns = shuji ? [
      ...
      },
      {
        ...
        render: (value, row, index) => {
          ...
          let message = '请填写数字!';
          return <FormItem style={{ marginBottom: 0 }} >
            {getFieldDecorator(`secTotal${index}`, {
              // initialValue: value,
              rules: [{
                required: !pattern.test(value),
                pattern,
              ...
              validateTrigger: 'onBlur',
            })(<Tooltip placement="right" title={value}><Input
              onClick={(e) =>{
                //抑制事件冒泡
                e.stopPropagation();
              }
              ...
              // allowClear
              // 打分角色	roleId	VARCHAR2		1|董事长;2|总裁;15|党委书记;4|战略企划部
              disabled={row.roleId !== "15"}
              // disabled={disabled}
              id={row?.orgId}
              value={value}
              onChange={(e) => {
                records[index].secTotal = e.target.value
                this.setState({
                  flag: !this.state.flag,
                })
              }}
