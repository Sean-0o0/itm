...
class ListButtonGroup extends React.Component {
  state = {
    ...
    //模版下载
    downloadUrl: '/OperateProcessor?operate=VO_RESOURCE_ALLOCATION_TMPL&Table=VO_RESOURCE_ALLOCATION',
  };
  ...
  onDownloadMessage = (messageObj) => { // iframe的回调事件
    if (!messageObj) { // 取消事件，对应 LiveBOS `operateCancel`
  ...
  submitModify = () => {
    ...
    const { newData, handReflsh } = this.props;
  ...
  render() {
    ...
    return <div style={{ marginTop: '1rem', marginBottom: '1rem', display: 'flex' }}>
      ...
      {ResourceAllocation.includes('ResourceAllocationTmpl')
        && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
          onClick={() => {
            this.setState({
      ...
      <LBFrameModal
        ...
        frameProps={{
          height: '40rem',
          src: `${localStorage.getItem('livebos') || ''}${downloadUrl}`,
          onMessage: this.onDownloadMessage,
        }}
      />
