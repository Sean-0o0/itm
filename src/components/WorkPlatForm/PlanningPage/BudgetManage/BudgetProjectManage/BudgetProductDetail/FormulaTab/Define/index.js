...
import { FetchQueryListBudgetFormula } from '../../../../../../../../services/planning/budgetManagement';
...
class Define extends React.Component {
  ...
  fetchDefineData = (payload) => {
    ...
    FetchQueryListBudgetFormula(payload).then((res) => {
      const { records = [] } = res;
      // if (oprType === '1') {
      const newList = []; // 营业部id
      ...
      <React.Fragment>
        <div className="clearfix m-szys-btn-list">
          <div className="left m-kequn-form-item">
            <span className="m-kequn-form-left">业务线</span>
            <TreeSelect
