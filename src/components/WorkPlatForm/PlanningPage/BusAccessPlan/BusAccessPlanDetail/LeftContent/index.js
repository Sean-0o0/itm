...
import {Tabs, message, Button, Modal, Drawer, Timeline, Input, Message, Row, Col, Icon, Radio, Form } from 'antd';
import BasicIndex from './BasicIndex';
import ModuleIndex from './ModuleIndex';
import config from '../../../../../../utils/config';
import {
  FetchAuditBusResponWorkflow, FetchQueryBusResponDetail, FetchQueryHisBusResponDetail,
...
const { api } = config;
const { planning: { createPlanWordForBusRespon } } = api;
const { TextArea } = Input;

class BusLeftContent extends React.Component {
  state = {
    headerInfo: '',
    assessInfo: {},
    planId: '',
  ...
  componentDidMount() {
    ...
    if (planType === '1' || planType === '2') {
      this.FetchQueryBusResponDetail(planId);
    }
  }
  ...
  componentWillReceiveProps(nextProps, nextContext) {
    if (nextProps !== this.props) {
      ...
      if (planType === '1' || planType === '2') {
        if (opr) {
          this.FetchQueryHisBusResponDetail(planId);
        } else {
          this.FetchQueryBusResponDetail(planId);
        }
      }
  ...
  //查询业务条线考核方案明细
  FetchQueryBusResponDetail = async (planId) => {
    await FetchQueryBusResponDetail({ planId }).then(res => {
  ...
  FetchQueryHisBusResponDetail = async (planId) => {
    await FetchQueryHisBusResponDetail({ planId }).then(res => {
      const { code = 0, note = '', result = {} } = res;
      if (code > 0) {
        const resultList = JSON.parse(result);
        this.setState({
  ...
  handleOk = () => {
    ...
    FetchAuditBusResponWorkflow(param).then(
      (res) => {
        const { code, note } = res;
        if (code > 0) {
  ...
  onOk = () => {
    ...
    //责任书导出pdf/word
    const actionUrl = createPlanWordForBusRespon;
    if (selectedKey === '') {
  ...
  render() {
    ...
    const { planType = '',planId = '',dictionary = [], buttons = [], stepId, drawerData, icon = false,icon2 =false, opr,rollback,headState={}} = this.props;
            ...
            <div className='advice-left-header clearfix'>
              ...
              <div style={{ float: 'right', marginRight: '20px' }}>
                ...
                {opr === false ? (
                  ...
                  <Button className='fcbtn m-btn-border m-btn-border-headColor btn-1c'
                          onClick={this.handleClick}>方案导出</Button>&nbsp;&nbsp;{rollback ===true?<Button className='fcbtn m-btn-border m-btn-border-headColor btn-1c'
                  ><Link to={{pathname:'/esa/planning/busResponListPage',query: { headState: headState }}}>返回</Link></Button>:''}</span>) : (opr === true ? (<Button
                  className='fcbtn m-btn-border m-btn-border-headColor btn-1c'
                  onClick={this.roolbackOldPage}>返回</Button>) : (
                    <span>
                  <Button className='fcbtn m-btn-border m-btn-border-headColor btn-1c'
                          onClick={this.handleClick}>方案导出</Button>&nbsp;&nbsp;{rollback ===true?<Button className='fcbtn m-btn-border m-btn-border-headColor btn-1c'
                    ><Link to={{pathname:'/esa/planning/busResponListPage',query: { headState: headState }}}>返回</Link></Button>:''}</span>))
                }
