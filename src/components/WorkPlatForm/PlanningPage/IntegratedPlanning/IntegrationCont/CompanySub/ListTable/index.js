...
class ListTable extends React.Component {
    ...
    fetchLBDelUrl = (e) => {
        FetchLivebosLink({
            method: 'TDEPARTMENT_PLAN_DELETE',
    ...
    fetchLBModUrl = (e) => {
        FetchLivebosLink({
            method: 'TDEPARTMENT_PLAN_UPDATE',
            object: 'TDEPARTMENT_PLAN',
            params: {
                ID: e
            },
    ...
    componentWillReceiveProps(nextProos) {
        const { authorities: { TDEPARTMENT_PLAN = [] } } = nextProos
        let columns = [];
        if (TDEPARTMENT_PLAN.includes("TDEPARTMENT_PLAN_UPDATE") || TDEPARTMENT_PLAN.includes("TDEPARTMENT_PLAN_DELETE")) {
            columns = [
                {
                ...
                {
                    title: <span style={{ fontSize: '1.2rem' }}>子分公司</span>,
                    dataIndex: 'orgname',
                    // width: '22.5%',
                    render: (text, row, index) => {
                        return <span style={{ fontSize: '1.2rem' }}>{text}</span>
                ...
                },
                {
                    ...
                    render: (text, row, index) => {
                        ...
                        return <Link to={'/esa/planning/CompanySubPlanning/' + EncryptBase64(JSON.stringify(param))}>
                    ...
                    render: (text, row, index) => {
                        ...
                        return <Popover content={datas.map((item) => {
                            return <p style={{ fontSize: '1.2rem' }}>
                                <a style={{ color: '#54A9DF' }} href={`${localStorage.getItem('livebos') || ''}/OperateProcessor?Column=FILES&PopupWin=false&Table=TDEPARTMENT_PLAN&operate=Download&Type=View&ID=${row.tid}&fileid=${item[0]}`}><Icon style={{ margin: '0 0.5rem' }} type="link" />{item[1]}
                                </a>
                            </p>;
                        })} title="附件列表">
                ...
                },
                {
                    ...
                    render: (text, row, index) => {
                        ...
                        node = <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                            {TDEPARTMENT_PLAN.includes("TDEPARTMENT_PLAN_UPDATE") &&
                            ...
                            {TDEPARTMENT_PLAN.includes("TDEPARTMENT_PLAN_DELETE") &&
                                <Button className="fcbtn m-btn-border  m-btn-border-headColor btn-1c" size="small" onClick={
