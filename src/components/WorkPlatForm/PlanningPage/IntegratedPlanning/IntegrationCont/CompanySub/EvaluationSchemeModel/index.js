...
import Color from 'react-virtualized-table/dist/Example/ColorHighlight';

const { TextArea } = Input;

class EvaluationSchemeModel extends React.Component {
  constructor(props) {
    super(props);
    this.state = {};
  }
              ...
              <BasicModal {...modalProps}>
                ...
                {this.props.modalProps.type === '3' ? null :
                  (<Form.Item style={{ margin: '1rem', padding: '1rem' }}
                              label={this.props.modalProps.type === '1' ? '意见征求人员:' : '考核小组成员:'} labelCol={{ span: 4 }}
                              wrapperCol={{ span: 20 }}>
                    <ShareInput
                ...
                <Form.Item style={{ margin: '1rem', padding: '1rem' }} label='提示' labelCol={{ span: 4 }}
                           wrapperCol={{ span: 20 }}>
                  {/*意见征求,提交审批,发布显示不同提示,,,发布时勾选数据中存在!===4的数据就显示【存在尚未审批完成的数据，是否确认发布？】 否则显示【发布后将不再允许考核方案修改及意见反馈，是否确认发布？】*/}
                  <div>{this.props.modalProps.type === '1' ? (<span
                    style={{ color: 'blue' }}>发起意见征求后，根据角色[反馈意见处理人]获取反馈意见的处理人员，可对反馈意见进行处理！</span>) : (this.props.modalProps.type === '2'?'提交审批后将不再允许考核方案修改及意见反馈，是否确认提交？':(this.props.modalProps.palnStatus.indexOf("0")||this.props.modalProps.palnStatus.indexOf("1") !==-1||this.props.modalProps.palnStatus.indexOf("2") !==-1||this.props.modalProps.palnStatus.indexOf("3") !==-1?'存在尚未审批完成的数据，是否确认发布？':'发布后将不再允许考核方案修改及意见反馈，是否确认发布？'))}</div>
                </Form.Item>
              </BasicModal>
