...
class ListHead extends React.Component {
    ...
    componentDidMount() {
        ...
        let asssessmentType = []
        // fetchObject('lborganization').then((res) => {
        //     asssessmentType = res.records
        //     this.setState({
        //         asssessmentType: asssessmentType,
        //     })
        // }).catch(err => {
        //     message.error(err)
        // })
        ...
        //     })
        // })
        this.fetchQueryOrgList(0)
        this.setState({
            // schemeType: schemeType,
            // schemeTypeStatus: schemeTypeStatus,
    ...
    handleSearch = (e) => {
        ...
        const headState = {
          ...
          FetchQueryAssessPlanList({
            ...
            "planType":schemeTypeId ? (schemeTypeId==='' ? '':schemeTypeId):(schemeTypeId === undefined?'':(params.schemeTypeId===''? '':params.schemeTypeId)),
      ...
      const { asssessmentType = [], year, schemeTypeStatusId, schemeTypeId, objId } = this.state
        ...
        return (
            <div className='clearfix' style={{ display: 'flex' }}>
                ...
                <div style={{ flexShrink: 0, fontSize: '1.1rem', fontWeight: '500' }} className='fl header-dept' >考核方案类型：
                    <Select style={{ width: '13rem' }} id='type' showSearch allowClear
                        defaultValue={schemeTypeId ? (schemeTypeId=== '' ? '':schemeTypeId):(schemeTypeId === undefined?'':(params.schemeTypeId=== ''? '':params.schemeTypeId))}
                        filterOption={(input, option) =>
                            option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                        onChange={(e) => { this.handleScheChange(e) }}>
                        {schemeType.map((item, index) => {
