...
class ListButtonGroup extends React.Component {
  ...
  render() {
    ...
    const { authorities: { assessPlanList = [] } } = this.props;
    ...
    return <div style={{ marginTop: '1rem', marginBottom: '1rem', display: 'flex' }}>
      {/* 判断用户是否具有按钮权限 */}
      {assessPlanList.includes('assessPlanLead')
      ...
      {assessPlanList.includes('addBussinessAssessment')
      ...
      {assessPlanList.includes('addDepartmentAssessPlan')
      ...
      {assessPlanList.includes('assessPlanConsultation')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
        ...
        意见征求</Button>}
      {assessPlanList.includes('assessPlanSubmitForReview')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
                 onClick={() => {
                   // this.fetchLBUrl(1, 'TASSESS_PLAN_BASEINFO_OPTION')
                   const { planIdStr } = this.props;
      ...
      {assessPlanList.includes('assessPlanSubmit')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
                 onClick={() => {
