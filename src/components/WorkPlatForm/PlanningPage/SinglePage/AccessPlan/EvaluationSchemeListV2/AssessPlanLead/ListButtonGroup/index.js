...
class ListButtonGroup extends React.Component {
  ...
  render() {
    ...
    const {palnStatus, authorities: { assessPlanLeadList = [] } } = this.props;
    ...
    return <div style={{ marginTop: '1rem', marginBottom: '1rem', display: 'flex' }}>
      {/* 判断用户是否具有按钮权限 */}
      {assessPlanLeadList.includes('AssessPlanLeadAdd')
      ...
      {assessPlanLeadList.includes('AssessPlanLeadOption')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
        ...
        意见征求</Button>}
      {assessPlanLeadList.includes('AssessPlanLeadSubmit')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
                 onClick={() => {
                   // this.fetchLBUrl(1, 'TASSESS_PLAN_BASEINFO_OPTION')
                   const { planIdStr } = this.props;
      ...
      {assessPlanLeadList.includes('AssessPlanLeadRelease')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
                 onClick={() => {
