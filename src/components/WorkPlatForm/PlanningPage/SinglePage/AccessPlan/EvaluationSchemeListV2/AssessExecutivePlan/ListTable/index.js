...
class ListTable extends React.Component {
    ...
    componentWillReceiveProps(nextProos) {
        ...
        const { authorities: { assessExecutivePlanList = [] }, reload = false, headState } = nextProos
        ...
        const columns = [
            {
                ...
                render: (text, row, index) => {
                    ...
                    return <Link to={'/esa/planning/AssessExecutivePlanDetail/' + EncryptBase64(JSON.stringify(param))}>
            ...
            },
            {
                ...
                render: (text, row, index) => {
                    ...
                    node = <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        {/* 通过auths的assessPlanLeadList数组以及该行的status字段共同决定是否具有按钮权限 */}
                        {/* 判断是否具有修改权限 */}
                        {assessExecutivePlanList.includes("AssessPlanLeadUpdate") &&
                        ...
                        {assessExecutivePlanList.includes("AssessPlanLeadDelete") && Number(row.status) !== -1
