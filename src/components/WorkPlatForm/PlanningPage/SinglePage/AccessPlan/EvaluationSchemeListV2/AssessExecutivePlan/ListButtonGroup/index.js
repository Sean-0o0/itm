...
class ListButtonGroup extends React.Component {
  ...
  render() {
    ...
    const {palnStatus, authorities: { assessExecutivePlanList = [] } } = this.props;
    ...
    return <div style={{ marginTop: '1rem', marginBottom: '1rem', display: 'flex' }}>
      {/* 判断用户是否具有按钮权限 */}
      {assessExecutivePlanList.includes('AssessPlanLeadAdd')
      && <Link to={`/esa/planning/bussinessAssessment/${EncryptBase64(JSON.stringify({ planType: 4 }))}`}>
      ...
      {assessExecutivePlanList.includes('AssessPlanLeadOption')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
        ...
        意见征求</Button>}
      {assessExecutivePlanList.includes('AssessPlanLeadSubmit')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
                 onClick={() => {
                   // this.fetchLBUrl(1, 'TASSESS_PLAN_BASEINFO_OPTION')
                   const { planIdStr } = this.props;
      ...
      {assessExecutivePlanList.includes('AssessPlanLeadRelease')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
                 onClick={() => {
                   const { planIdStr } = this.props;
                   const flag = planIdStr.substring(planIdStr.indexOf('=') + 1).length > 0 ? true : false;
