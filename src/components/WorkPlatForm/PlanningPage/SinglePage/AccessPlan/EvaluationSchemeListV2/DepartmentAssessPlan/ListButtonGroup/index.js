...
class ListButtonGroup extends React.Component {
  ...
  render() {
    ...
    const {palnStatus, authorities: { assessPlanDepList = [] } } = this.props;
    ...
    return <div style={{ marginTop: '1rem', marginBottom: '1rem', display: 'flex' }}>
      {/* 判断用户是否具有按钮权限 */}
      {/*{assessPlanDepList.includes('assessPlanLead')*/}
      ...
      {/*</Link>*/}
      {/*}*/}
      {/*{assessPlanDepList.includes('addBussinessAssessment')*/}
      {/*&& <Link to={`/esa/planning/bussinessAssessment/${EncryptBase64(JSON.stringify({ planType: 2 }))}`}>*/}
      {/*  <Button style={{ flexShrink: 0, marginRight: '10px' }}*/}
      {/*          className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'>新增业务条线考核方案</Button>*/}
      {/*</Link>*/}
      {/*}*/}
      {assessPlanDepList.includes('AssessPlanDepAdd')
      ...
      {assessPlanDepList.includes('AssessPlanDepOption')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
        ...
        意见征求</Button>}
      {assessPlanDepList.includes('AssessPlanDepSubmit')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
                 onClick={() => {
                   // this.fetchLBUrl(1, 'TASSESS_PLAN_BASEINFO_OPTION')
                   const { planIdStr } = this.props;
      ...
      {assessPlanDepList.includes('AssessPlanDepRelease')
      && <Button className='fcbtn m-btn-border m-btn-middle m-btn-border-headColor btn-1c'
                 onClick={() => {
