...
class ListTable extends React.Component {
    ...
    componentWillReceiveProps(nextProos) {
        ...
        const { authorities: { assessPlanDepList = [] }, reload = false, headState } = nextProos
        ...
        const columns = [
            ...
            // },
            {
                ...
                render: (text, row, index) => {
                    ...
                    return <Link to={'/esa/planning/DepaccessPlanDetail/' + EncryptBase64(JSON.stringify(param))}>
            ...
            },
            {
                ...
                render: (text, row, index) => {
                    ...
                    node = <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        {/* 通过auths的assessPlanDepList数组以及该行的status字段共同决定是否具有按钮权限 */}
                        {/* 判断是否具有修改权限 */}
                        {assessPlanDepList.includes("AssessPlanDepUpdate") &&
                        ...
                        {/* 判断是否具有删除权限 */}
                        {/* {assessPlanDepList.includes("assessPlanDelete") && (Number(row.status) < 2 && Number(row.status) !== -1)
                        ...
                        {assessPlanDepList.includes("AssessPlanDepDelete") && Number(row.status) !== -1
                            && <Button className="fcbtn m-btn-border  m-btn-border-headColor btn-1c" size="small" onClick={
                                (e) => { this.fetchLBUrl(row.planId) }}>删除</Button>}
        ...
        return (
            <div style={{ marginTop: '1rem' }}>
                {data.length > 0 ? <Table
                    ...
                    key={'1'}
                    bodyStyle={{ minHeight: 'calc(100vh - 24.5rem)' }}
                ...
                /> : <Table
                    ...
                    rowSelection={rowSelection} columns={columns} dataSource={data}
                    key={'1'}
                    // bodyStyle={{ minHeight: 'calc(100vh - 27.5rem)' }}
                    rowKey={record => record.planId}
                    scroll={{ x: 1300 }}
                />}
                <LBFrameModal
