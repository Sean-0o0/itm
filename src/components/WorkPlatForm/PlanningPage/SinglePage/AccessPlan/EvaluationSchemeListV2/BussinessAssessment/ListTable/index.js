...
class ListTable extends React.Component {
    ...
    componentWillReceiveProps(nextProos) {
        ...
        const { authorities: { assessPlanBusList = [] }, reload = false, headState } = nextProos
        ...
        const columns = [
            ...
            // },
            {
                ...
                render: (text, row, index) => {
                    ...
                    return <Link to={'/esa/planning/BusaccessPlanDetail/' + EncryptBase64(JSON.stringify(param))}>
            ...
            },
            {
                ...
                render: (text, row, index) => {
                    ...
                    node = <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        {/* 通过auths的assessPlanBusList数组以及该行的status字段共同决定是否具有按钮权限 */}
                        {/* 判断是否具有修改权限 */}
                        {assessPlanBusList.includes("AssessPlanBusUpdate") &&
                        ...
                        {/* 判断是否具有删除权限 */}
                        {/* {assessPlanBusList.includes("assessPlanDelete") && (Number(row.status) < 2 && Number(row.status) !== -1)
                        ...
                        {assessPlanBusList.includes("AssessPlanBusDelete") && Number(row.status) !== -1
                            && <Button className="fcbtn m-btn-border  m-btn-border-headColor btn-1c" size="small" onClick={
                                (e) => { this.fetchLBUrl(row.planId) }}>删除</Button>}
