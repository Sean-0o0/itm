...
import AddIndFoot from '../AppointIndFoot'
import {
  FetchQueryIndiList,
  FetchQueryUserList,
...
class AppointIndBody extends React.Component {
    ...
    componentWillMount() {
        const { orgId = '', mon = '', planId = '',yr = '' } = this.props;
        //考核跟踪方案明细查看
      FetchQueryAssessPlanBusDetail({
            planId: planId,
        }).then(res => {
            ...
            if (code > 0) {
                ...
                console.log("resultList-chenjian",resultList)
                let data = []
                let keyword = []
                if(resultList.result1.length > 0){
                  resultList.result1.forEach(item =>{
                    let param = {
                      INDI_ClASS_NAME: item.INDI_CLASS || '',
                      INDI_TYPE: item.INDI_TYPE || '',
                ...
                if(resultList.result2.length > 0){
                  resultList.result2.forEach(item =>{
                    let param = {
                      INDI_ID: item.INDI_ID || '',//指标ID
                      KEYWORK: item.ASSESS_NOTE || '',
                      WEIGHT: item.WEIGHT || '',//权重
                      // REMARK: item.REMARK || '',//备注说明
                      // OPR_EMP: item.OPR_EMP || '',//录入人
                    }
    ...
    componentWillReceiveProps(nextProps) {
        const { orgId: nextOrgId = '', mon: nextMon = '', planId: nextPlanId = '',yr:nextYr = '' } = nextProps;
        const { orgId = '', mon = '', planId = '',yr = '' } = this.props;
        if (nextOrgId !== orgId || nextYr !== yr){
            //考核跟踪方案明细查看
            FetchQueryAssessPlanBusDetail({
                planId: planId,
            }).then(res => {
