...
import { FetchQueryBudgetFormulaConf } from '../../../../../services/planning/budgetManagement';
...
class RightContent extends React.Component {
  ...
  componentDidMount = () => {
    ...
    this.FetchQueryBudgetFormulaConf(dataTier, calMode, grpList);
  }
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.dataTier !== this.props.dataTier) {
      ...
      this.FetchQueryBudgetFormulaConf(nextProps.dataTier, calMode, grpList);
    } else if (nextProps.calMode !== this.props.calMode) {
      const { dataTier = 1 } = nextProps;
      this.FetchQueryBudgetFormulaConf(dataTier, nextProps.calMode, grpList);
  ...
  FetchQueryBudgetFormulaConf = async (dataTier = 1, calMode = [], grpList = []) => {
      ...
      for (const item of calMode) {
        for (const grpItem of grpList) {
          if (item === Number(grpItem.ibm)) {
            res = await FetchQueryBudgetFormulaConf({ // 客户月度统计
        ...
        if (item === 0) {
          monthCountRes = await FetchQueryBudgetFormulaConf({ // 客户月度统计
            confPrjType: 0,
            dataTier,
            version: versionId,
          });
        ...
        if (item === -1) {
          salaryItemsRes = await FetchQueryBudgetFormulaConf({ // 薪酬项目
            confPrjType: -1,
            dataTier,
            version: versionId,
          });
        ...
        if (item === -2) {
          sysFunctionRes = await FetchQueryBudgetFormulaConf({ // 系统函数
            confPrjType: -2,
            dataTier,
            version: versionId,
          });
          ...
          >
            ...
            }
            {
              grpList.length > 0 && calMode.indexOf(-1) !== -1 ?
                (
                  <TreeNode title="预算项目" key="key-3" selectable={false}>
