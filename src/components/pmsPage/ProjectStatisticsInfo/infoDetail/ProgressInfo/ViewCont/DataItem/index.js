      ...
      {fxxxItem.length ?
        ...
        >
          {fxxxItem.map((item, index) => {
            ...
            return <div className='risk-item-box'
                        style={{borderBottom: index !== fxxxItem.length - 1 ? '1px solid #EBEEF5' : 'none'}}>
              <div className='risk-item-header flex-r'>
                <div className='risk-item-title  flex1'>风险{index + 1}</div>
                <div className='risk-item-status' style={{
                  background: ZT === 2 ? '#EAEFFF' : '#F5F7FA',
                  color: ZT === 2 ? '#3361FF' : '#909399'
                }}>▪ {ZT === 2 ? '已处理' : '未处理'}</div>
              </div>
        ...
        <div className='data-item'>
          ...
          <div className='data-box'>
            {matterInfos.length ?
              ...
              <React.Fragment>
                <div
                  style={{width: '100%', paddingBottom: '2rem', textAlign: 'center', color: 'rgb(144, 147, 153)'}}>暂无数据
                </div>
              </React.Fragment>
            }
          </div>
