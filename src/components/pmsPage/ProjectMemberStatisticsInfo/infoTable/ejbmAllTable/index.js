...
export default function EjbmAllTable(props) {
  ...
  return activeKeyFlag ? (
    tableDataLD.length > 0 || tableDataBM.length > 0 ? (
      <Spin spinning={loading} wrapperClassName="spin" tip="正在努力的加载中..." size="large">
        <div className="info-table">
          {tableDataLD.length > 0 && <div className="info-table-title">领导统计</div>}
          {compareRYVisible && (
            <DataComparisonRY
              userId={userId}
              closeModal={closeCompareRYVisibleModal}
              visible={compareRYVisible}
              defaultYear={defaultYear}
            />
          )}
          <div className="info-table-content">
            {tableDataLD.length > 0 &&
              tableDataLD.map(item => {
                return (
                  <div style={{ width: itemWidth }} className="info-table-content-box">
                    <div className="info-table-content-box-title">
                      ...
                      <div
                        className="info-table-content-box-title-right"
                        onClick={() => getCompareRYModel(item)}
                      >
                        <i className="iconfont icon-vs" onClick={() => getCompareRYModel(item)} />
                        数据对比
                      </div>
                    </div>
                    <Link
                    ...
                    >
                      <div
                        className="info-table-content-box-radar"
                        onMouseEnter={() => getFooterLD(item.USERID)}
                      ...
                      >
                        ...
                        >
                          负责项目:&nbsp;&nbsp;
                          <span style={{ color: '#303133', fontWeight: 500 }}>{item.XMZS}</span>
                        </span>
                        <ReactEchartsCore
                          echarts={echarts}
          ...
          {tableDataBM.length > 0 && <div className="info-table-title">部门统计</div>}
          {compareBMVisible && (
            <DataComparisonBM
              orgId={orgId}
              closeModal={closeCompareBMVisibleModal}
              visible={compareBMVisible}
              defaultYear={defaultYear}
            />
          )}
          <div className="info-table-content">
            {tableDataBM.length > 0 &&
              tableDataBM.map((item, index) => {
                return (
                  <div style={{ width: itemWidth }} className="info-table-content-box">
                    <div className="info-table-content-box-title">
                      <div className="info-table-content-box-title-left">{item.ORGNAME}</div>
                      <div
                        className="info-table-content-box-title-right"
                        onClick={() => getCompareBMModel(item)}
                      >
                        <i className="iconfont icon-vs" onClick={() => getCompareBMModel(item)} />
                        数据对比
                      </div>
                    </div>
                    <span
                    ...
                    <div
                      className="info-table-content-box-radar"
                      onMouseEnter={() => getFooterBM(item.ORGID)}
                    ...
                    >
                      ...
                      {footerVisableBM === item.ORGID && (
                        <div className="info-table-content-box-footer">
                          ...
                          <div
                            className="info-table-content-box-footer-right"
