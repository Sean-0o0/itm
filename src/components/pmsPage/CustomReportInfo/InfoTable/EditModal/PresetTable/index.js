...
class PresetTable extends React.Component {
  ...
  callbackData = (tableData) => {
    ...
    console.log("newDataSourcenewDataSource", newDataSource)
  ...
  handleSave = row => {
    ...
    console.log("newDatanewData-1111", newData)
  ...
  render() {
    ...
    const _this = this;
    if (columns.filter(item => item.dataIndex === 'GLXM').length === 0) {
      columns.push({
        title: '关联项目',
        dataIndex: 'GLXM',
        // width: '15%',
        ...
        render(text, record, index) {
          return (<Select filterOption={(input, option) =>
              option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                          ...
                          value={record['GLXM' + record.ID] !== '-1' ? record['GLXM' + record.ID] : ''}
                          defaultValue={record['GLXM' + record.ID] !== '-1' ? record['GLXM' + record.ID] : ''}
      ...
      columns.push({
        ...
        render(text, record, index) {
          return (<Select filterOption={(input, option) =>
              option.props.children.toLowerCase().indexOf(input.toLowerCase()) >= 0}
                          showSearch allowClear style={{width: '100%'}}
                          value={record['TXR' + record.ID] !== '-1' ? record['TXR' + record.ID] : ''}
                          defaultValue={record['TXR' + record.ID] !== '-1' ? record['TXR' + record.ID] : ''}
