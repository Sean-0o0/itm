  ...
  handleSearch = () => {
    ...
    QueryAttachLibraryList({
      ...
      .then((res = {}) => {
        ...
        if (code > 0) {
          this.setState({
            selectedRows: record.filter(x => x.zxwd === ''),
            selectedRowKeys: record.filter(x => x.zxwd === '').map(item => item.wdid),
          });
        }
      })
      .catch(e => {
  ...
  render() {
    ...
    const rowSelection = {
      ...
      onSelectAll: (selected, selectedRows, changeRows) => {
        console.log(
          '🚀 ~ file: index.js:255 ~ InfoTable ~ render ~ selected, selectedRows, changeRows:',
          selected,
          selectedRows,
          changeRows,
        );
          ...
          if (record.zxwd !== '') {
            return (
              <Tooltip
                title={record.zxwdmc}
                placement="topLeft"
                onClick={() => window.open(record.zxwd)}
              >
                <a className="opr-btn">{record.zxwdmc}</a>
              </Tooltip>
            );
          } else if (text) {
            const { wdid = '' } = record;
            ...
            let content = (
              <div className="fj-box">
                <div className="fj-header">
                  ...
                  <div className="fj-header-btn" onClick={() => this.downlownRow(items, wdid)}>
                    全部下载
                  </div>
                </div>
                {items.length ? (
                    ...
                    {items.map((item, index) => {
                      ...
                      return (
                        <div className="fj-item flex-r">
                          ...
                          <div className="fj-btn" onClick={() => this.downlown(id, title, wdid)}>
          ...
          if (record.lcId !== '-') {
            return (
              <div
                className="opr-btn"
                onClick={() => {
                  this.handleModifyVisible(record);
                }}
              >
                查看历史
              </div>
