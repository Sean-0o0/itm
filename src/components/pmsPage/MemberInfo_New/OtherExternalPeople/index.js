 ...
 * @returns 
 */
const OtherExternalPeople = (props) => {

  const { getFieldDecorator, getFieldsValue, resetFields } = props.form

  const { rymcData, responsiblePeopleData, gysData, } = useContext(MemberInfoContext)
  ...
  const queryHandle = async (resetCurpageBool, extraParams) => {
    ...
    try {
      ...
      message.error(`查询其他外部人员数据失败,${!err.success ? err.message : err.note}`, 3)
      setIsTableLoading(false)
    }
  }
  ...
  return (
    <Form>
      <div className='MemberInfo_OtherExternalPeople Tab234Pages'>
        ...
        <div className='filterBar'>
          <Row className='row1' style={rowStyle} gutter={[gridProps.horizontalGutter, gridProps.verticalGutter]}>
            ...
            <Col span={gridProps.colSpan}>
              <Form.Item label="负责人" {...formItemProps}>
                {getFieldDecorator(`fzr`, {
                  ...
                  <Select
                    ...
                    placeholder="请选择"
                  >
                    {responsiblePeopleData.map((item, index) => (
...
export default Form.create()(OtherExternalPeople)
