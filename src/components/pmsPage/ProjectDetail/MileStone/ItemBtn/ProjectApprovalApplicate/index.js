...
export default connect(({ global }) => ({
  ...
  Form.create()(function ProjectApprovalApplicate(props) {
    ...
    const { userBasicInfo = {}, dictionary = {}, visible, setVisible, form,
      prjBasic, refresh, xmbh, currentXmid
    } = props;
    ...
    const [isConfirmBoxShow, setIsConfirmBoxShow] = useState(false) // 超额预算弹窗警告
    ...
    const [historyBudget, setHistoryBudget] = useState(null) //历史立项流程总金额（查询后计算得出）
    const [historyBudgetTableData, setHistoryBudgetTableData] = useState([]) //历史立项流程——表格数据
    ...
    */
    function judgeMoneyHanlde(bundaryMoney) {
      ...
      const b = parseFloat(prjBasic.KJCGJE || 0).toFixed(2);
    ...
    const submitHandle = () => {
      validateFields(async (err, values) => {
        if (err === null) {
          ...
          // 历史流程预算 + 用户填写的预算 >总预算， 则弹窗提示进行作废操作
          if (historyBudget + filledBudget > allBudget) {
            setIsConfirmBoxShow(true)
            return;
          }
          ...
          <Form className="content-box">
            ...
            {isConfirmBoxShow &&
              <ConfirmBox
                dataObj={dataObj}
                form={form}
                isConfirmBoxShow={isConfirmBoxShow}
                setIsConfirmBoxShow={setIsConfirmBoxShow}
                historyBudget={historyBudget}
                setHistoryBudget={setHistoryBudget}
                historyBudgetTableData={historyBudgetTableData}
                setHistoryBudgetTableData={setHistoryBudgetTableData}
              />
            }
